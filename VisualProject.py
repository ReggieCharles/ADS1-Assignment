...
"""Replaced the gender column to show 'Male' & 'Female' 
instead of 'M' & 'F' for clearer identification"""
...
def create_lineplot(data):
    """Created a function to plot a line graph shwowing the 
    profit over the years"""
    
    plt.figure(figsize=(9.0, 6.0))
    plt.plot(data, linestyle='-', color='black', label='Profit', 
...
"""Createda list to input the different months and their respective
profit and revenue in order"""
...
#Plotted the profit and revenue against the month
create_lineplot2(monthly_revenue['Month'], monthly_revenue['Revenue'], 
                 monthly_revenue['Profit'])

#Implemented a value count on the vaeious age groups
Ag_dist = bike['Age_Group'].value_counts().sort_values(ascending = False)

"""Created a function to plot a piechart to compare the 
...
#summed up the total profits and grouped by the product category
P = bike.groupby('Product_Category').sum()['Profit'].sort_values(
    ascending = False)

"""Created a function to plot a bar chart to show the comparison between
the product categories with respect to profits"""
